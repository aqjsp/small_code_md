...
## 2. 开发环境
- 编译器：GCC（开源，跨平台）、Clang（LLVM项目，现代优化）、MSVC（Windows专用）。
- 安装示例（Windows）：
  - MinGW-w64：下载安装器，选择x86_64架构，添加bin到PATH。
  - Visual Studio：安装社区版，选择C++桌面开发工作负载。
- IDE配置：VSCode安装C/C++扩展，配置tasks.json与launch.json用于构建与调试。
- 常见问题：环境变量未设置、缺少依赖库。
...
## 2. 开发环境
- 编译器：GCC（开源，跨平台）、Clang（LLVM项目，现代